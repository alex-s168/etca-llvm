...
// RUN: %clang --target=etca-unknown-elf -mcpu=generic -m32bit -mptr32 -mdw -nostdlib %s -### 2>&1 | FileCheck %s --check-prefix=LLD-32
// RUN: %clang --target=etca-unknown-elf -mcpu=generic -m64bit -mptr64 -mdw -mqw -nostdlib %s -### 2>&1 | FileCheck %s --check-prefix=LLD-64
//
...
// RUN: %clang --target=etca-unknown-elf -mcpu=generic -m64bit -mptr64 -mdw -mqw -fuse-ld=lld -nostdlib %s -### 2>&1 | FileCheck %s --check-prefix=LLD-64
