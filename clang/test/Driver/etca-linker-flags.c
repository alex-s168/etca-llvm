...
// RUN: %clang --target=etca-unknown-elf -mcpu=generic -m32bit -mptr32 -nostdlib %s -### 2>&1 | FileCheck %s --check-prefix=LLD-32
// RUN: %clang --target=etca-unknown-elf -mcpu=generic -m64bit -mptr64 -nostdlib %s -### 2>&1 | FileCheck %s --check-prefix=LLD-64
//
...
// RUN: %clang --target=etca-unknown-elf -mcpu=generic -m64bit -mptr64 -fuse-ld=lld -nostdlib %s -### 2>&1 | FileCheck %s --check-prefix=LLD-64
