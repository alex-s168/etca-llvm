...
## 1. CRITICAL — G_SELECT Passes `$dst` as Condition Instead of `$cond`

**File:** `GISel/ETCAInstructionSelector.cpp`  
**Line:** ~392 (G_SELECT handler)

The SELECT_Pseudo instruction is defined as:

```
def SELECT_Pseudo : EInst<(outs GPR:$dst), (ins GPR:$cond, GPR:$t, GPR:$f, i16imm:$pred), ...>;
```

But the instruction selector emits:

```cpp
BuildMI(MBB, MI, MIMD, TII.get(ETCA::SELECT_Pseudo), Dst)
    .addReg(Dst)          // ← BUG: should be .addReg(Cond)!
    .addReg(TrueVal)
    .addReg(FalseVal)
    .addImm(PredVal);
```

The condition operand is incorrectly set to `Dst` (the result register) instead of `Cond` (the actual condition register). This means:

- **Always-True Selects**: When the condition happens to be non-zero (most common), the select always picks the true value.
- **Wrong Branch in ETCASelectExpand**: The expand pass reads operand 0 as the destination and doesn't use the condition register at all — because it assumes CMP was already emitted. But if CMP was emitted separately (Case 1 or Case 2), the emitted branch tests the wrong condition value.
- **Potentially-correct with ICMP_Pseudo**: If G_SELECT's cond is defined by ICMP_Pseudo, CMP was already emitted and flags are set correctly. The branch instruction reads the flags, not the register. But `Dst` as a reg operand to SELECT_Pseudo is still wrong — the expander uses `$dst` twice (once as result, once as tied source).

**Fix:** `.addReg(Dst)` → `.addReg(Cond)`.

---

## 2. CRITICAL — `etca32p64` CPU Model Has Incorrect Feature Set

**File:** `ETCA.td` (ProcessorModel definition)
...
The name `etca32p64` implies **32-bit word + 64-bit pointer**. The clang driver's `ETCATargetInfo.cpp` defines:

```cpp
CK_ETCA32P64, // 32b word + 64b ptr  (32-bit ops on 64-bit regs)
```

And sets `WordSize=32, PtrSize=64`. But the backend CPU model uses `Feature64Bit` which sets `WordSize=64`. This causes:

- **DataLayout mismatch** between clang (which calculates `sizeof(int)=4`) and LLVM backend (which uses 64-bit registers, `-S64`, native integers `n8:16:32:64`).
- **Register width mismatch**: `getRegWidth()` returns 64, but operations are 32-bit.
- **Stack slot sizing mismatch**: `getStackAlign()` returns 8 bytes instead of 4 bytes.
- **Calling convention mismatch**: promotes i16 return values to i64 (hasQW) instead of i32 (hasDW).

**Fix:** Use `Feature32Bit, FeaturePtr64, FeatureDW, FeatureDWAS` instead of `Feature64Bit, FeaturePtr64, FeatureDW, FeatureQW, FeatureQWAS`.
