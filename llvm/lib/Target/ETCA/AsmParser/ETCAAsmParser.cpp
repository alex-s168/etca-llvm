...
const ETCAAsmParser::BranchEntry ETCAAsmParser::BranchOps[] = {
    ...
    {"jne", ETCA::BNE},
    {"jnz", ETCA::BNE},
    // Direct flag tests (new: not generated by LLVM but available in asm)
    {"bn", ETCA::BN},
    {"bnn", ETCA::BNN},
    {"bov", ETCA::BOV},
    {"bnov", ETCA::BNOV},
...
bool ETCAAsmParser::matchAndEmitInstruction(SMLoc IDLoc, unsigned &Opcode,
  ...
  if (FE) {
    ...
    if (FE->IsTargetExpr) {
      ...
      // Uses CurrentInstrWidth from size suffix (h=8, x=16, d=32, q=64)
      // to select the width-specific instruction.
      auto &Op = static_cast<ETCAOperand &>(*Operands[1]);
      if (Op.isReg()) {
        unsigned Reg = Op.getReg();
        // Select width-specific PUSH based on suffix first, else register
        // class.
        if (CurrentInstrWidth == 8)
          Inst.setOpcode(ETCA::PUSH8);
        else if (CurrentInstrWidth == 32 ||
                 (CurrentInstrWidth == 0 && Reg >= ETCA::D0 && Reg <= ETCA::D7))
          Inst.setOpcode(ETCA::PUSH32);
        else if (CurrentInstrWidth == 64 ||
                 (CurrentInstrWidth == 0 && Reg >= ETCA::Q0 && Reg <= ETCA::Q7))
      ...
      // POP <reg> — select width-specific POP based on suffix or register
      // class.
      auto &Op = static_cast<ETCAOperand &>(*Operands[1]);
      if (!Op.isReg())
