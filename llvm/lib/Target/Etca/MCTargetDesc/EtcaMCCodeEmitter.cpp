...
// getMachineOpValue - Return binary encoding of operand. If the machine
// operand requires relocation, record the relocation and return zero.
unsigned EtcaMCCodeEmitter::getMachineOpValue(
    const MCInst &Inst, const MCOperand &MCOp, SmallVectorImpl<MCFixup> &Fixups,
    const MCSubtargetInfo &SubtargetInfo) const {
  if (MCOp.isReg())
    return getEtcaRegisterNumbering(MCOp.getReg());
  if (MCOp.isImm())
  ...
  assert(isa<EtcaMCExpr>(Expr) || Expr->getKind() == MCExpr::SymbolRef);
...
static unsigned adjustPqBits(const MCInst &Inst, unsigned Value,
  ...
  if (!LPAC::isPostOp(AluCode) &&
      ((Op2.isImm() && Op2.getImm() != 0) ||
       (Op2.isReg() && Op2.getReg() != Etca::R0) || (Op2.isExpr())))
    Value |= (1 << PBitShift);
...
unsigned
EtcaMCCodeEmitter::adjustPqBitsRmAndRrm(const MCInst &Inst, unsigned Value,
                                         const MCSubtargetInfo &STI) const {
  return adjustPqBits(Inst, Value, 17, 16);
}

unsigned
EtcaMCCodeEmitter::adjustPqBitsSpls(const MCInst &Inst, unsigned Value,
                                     const MCSubtargetInfo &STI) const {
...
// Encode Etca Memory Operand
unsigned EtcaMCCodeEmitter::getRiMemoryOpValue(
  ...
  Encoding = (getEtcaRegisterNumbering(Op1.getReg()) << 18);
  if (Op2.isImm()) {
...
unsigned EtcaMCCodeEmitter::getRrMemoryOpValue(
    const MCInst &Inst, unsigned OpNo, SmallVectorImpl<MCFixup> &Fixups,
    const MCSubtargetInfo &SubtargetInfo) const {
  unsigned Encoding;
  const MCOperand Op1 = Inst.getOperand(OpNo + 0);
  ...
  assert(Op1.isReg() && "First operand is not register.");
  Encoding = (getEtcaRegisterNumbering(Op1.getReg()) << 15);
  assert(Op2.isReg() && "Second operand is not register.");
  Encoding |= (getEtcaRegisterNumbering(Op2.getReg()) << 10);

  assert(AluMCOp.isImm() && "Third operator is not immediate.");
  // Set BBB
  unsigned AluOp = AluMCOp.getImm();
  Encoding |= LPAC::encodeEtcaAluCode(AluOp) << 5;
  // Set P and Q
...
unsigned
EtcaMCCodeEmitter::getSplsOpValue(const MCInst &Inst, unsigned OpNo,
                                   SmallVectorImpl<MCFixup> &Fixups,
                                   const MCSubtargetInfo &SubtargetInfo) const {
  unsigned Encoding;
  const MCOperand Op1 = Inst.getOperand(OpNo + 0);
  ...
  Encoding = (getEtcaRegisterNumbering(Op1.getReg()) << 12);
  if (Op2.isImm()) {
...
unsigned EtcaMCCodeEmitter::getBranchTargetOpValue(
    const MCInst &Inst, unsigned OpNo, SmallVectorImpl<MCFixup> &Fixups,
    const MCSubtargetInfo &SubtargetInfo) const {
  ...
  Fixups.push_back(MCFixup::create(
      0, MCOp.getExpr(), static_cast<MCFixupKind>(Etca::FIXUP_LANAI_25)));
