...
; Test that G_VAARG and G_VASTART are correctly lowered by the legalizer.
; ETCA uses CharPtrBuiltinVaList (va_list = char*).
;
; G_VASTART stores the address of the varargs save area into the va_list ptr.
; G_VAARG loads the value from *ap, advances ap by argument size, stores ap
; back, and returns the loaded value.
...
; The varargs save area is a stack object (size 6 = 3 regs × 2 bytes).
; LEGAL:      stack:
; LEGAL:        - { id: 0, name: '', type: default, offset: 0, size: 6, alignment: 2

; G_VASTART: store address of save area (%stack.0) into the alloca'd ptr.
...
; G_VAARG: load ap → advance by 2 → store ap → load value.
